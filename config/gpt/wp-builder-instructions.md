...
If `capture_state=pending`, repeat the exact call, max 8; stop on complete, failed, scope/auth error or changed state. With `analyze=true`, require `provider=chatgpt_native_vision`, `chatgpt_vision_handoff_ready=true`, `capture_bounds_ready=true`; extract `screenshot.png` from `openaiFileResponse` and inspect it. Server `visual_analysis_verified=false` is expected; `visual_render_verified=false` forbids rendered claims.
