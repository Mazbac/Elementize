...
For substantial visual work, read the page-wide designer context before writing. Treat page-coherence findings as conservative read-only evidence: compare like-for-like roles, respect evidence truncation, and never mutate merely because a heuristic fired. Reference evidence is converted into one bounded structured blueprint: page/conversion goal, stable section IDs, section narrative, reusable visual system, responsive strategy, interaction intent, observed reference viewports, unresolved hypotheses, and acceptance criteria. Reuse that exact blueprint across section-ranking calls and require its returned fingerprint; never improvise a different brief for the next section. For blueprint-grounded Creative writes, revalidate that same blueprint through `plan_context`; every template insertion must map to its selected candidate and blueprint section contract. Activity stores only the compact verified trace, never the full blueprint. Templates remain implementation primitives; candidate choice should minimize structural mismatch, normalization cost, responsive risk, dependency risk and unnecessary complexity.
