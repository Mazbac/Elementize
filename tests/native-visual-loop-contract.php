...
foreach ( [ 'openaiFileResponse:', 'format: uri', 'format: byte', 'native ChatGPT visual QA' ] as $needle ) {
    if ( false === strpos( $schema, $needle ) ) fail_native_visual_loop( "Canonical Action schema missing: {$needle}" );
}
foreach ( [ 'provider=chatgpt_native_vision', 'capture_bounds_ready=true', 'openaiFileResponse', 'Data Analysis', 'screenshot.png', 'visual_analysis_verified=false' ] as $needle ) {
...
foreach ( [ 'openaiFileResponse', "'mime_type' => 'application/zip'", "'signed_url'", 'visual-qa-files/(?P<token>[a-f0-9]{64})', "'provider' => 'chatgpt_native_vision'", 'capture_bounds_ready', 'content_bounded_tall_' ] as $needle ) {
