...
Write-Host ("[INFO] Aesthetic Brain: {0}; hardening: {1}" -f $status.aesthetic_brain_version, $status.aesthetic_brain_hardening_version)
...
[pscustomobject]@{
    ...
    ProfessionalApproval = [string]$art.professional_approval
...
if ($art.available -and $art.approval_usable) {
    Write-Host '[PASS] Aesthetic Brain produced a complete, reviewable page-level judgment.' -ForegroundColor Green
}
elseif ($art.available) {
    Write-Host '[WARN] Aesthetic Brain returned a judgment, but coverage/confidence is not yet sufficient to treat approval as usable.' -ForegroundColor Yellow
}
else {
